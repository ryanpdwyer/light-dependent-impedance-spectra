...
Rdarks = np.array([1e5, 1e7, 1e8])
Rlights = np.array([10])
tau_lights = np.array([10.0, 20, 40, 80, 160])
Ndec_resps = [2, 4, 4, 8, 12]
V0_fracs = np.array([0.7, 1.0]) # It doesn't make physical sense for V0_frac to be less than Cs/(Cs + Ct)...
phases = np.array([0, np.pi])
Vs = [Voltage(10.0, x) for x in [48.0, 96.0, 192.0, 384.0]]
Czs = -np.array([3.5e-05, 3.5e-05*np.sqrt(2), 3.5e-05*0.8])
Css = np.array([0.1e-4, 0.4e-4, 1e-4])
A0s = np.array([0.050])
